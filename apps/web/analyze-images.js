...
console.log(`Images utilisées: ${usedImages.length} (${(usedImages.length/allImages.length*100).toFixed(1)}%)`);
console.log(`Images inutilisées: ${unusedImages.length} (${(unusedImages.length/allImages.length*100).toFixed(1)}%)`);
console.log(`Taille totale: ${formatSize(totalSize)}`);
console.log(`Taille utilisée: ${formatSize(usedSize)} (${(usedSize/totalSize*100).toFixed(1)}%)`);
console.log(`Taille inutilisée: ${formatSize(unusedSize)} (${(unusedSize/totalSize*100).toFixed(1)}%)`);
console.log(`\n📊 LOI DE PARETO:`);
console.log(`Les ${top20PercentCount} images les plus utilisées (20%) représentent ${paretoRatio}% des références`);

console.log('\n\nIMAGES UTILISÉES (triées par fréquence)');
console.log('========================================\n');
...
usedImages.forEach((img, index) => {
  ...
  console.log(`   Références: ${img.count}`);
  console.log(`   Utilisée dans:`);
...
console.log('\n\nIMAGES INUTILISÉES (triées par taille)');
console.log('=======================================\n');
...
console.log('================================\n');
console.log(`💾 Espace libérable: ${formatSize(unusedSize)}`);
console.log(`📁 ${unusedImages.length} images peuvent être supprimées\n`);
...
console.log('\n✅ Rapport détaillé sauvegardé dans: image-analysis-report.json');
